...
try {
  ...
  if (script === 'jest') {
    result = runCommand(
      "npx cross-env NODE_OPTIONS=--max_old_space_size=4096 jest --testRunner='jest-circus/runner' --detectOpenHandles -w 1 --ci --forceExit --config node_modules/@emarketeer/ts-microservice-commons/dist/jest.config.json",
      scriptArgs
    )
  }
} finally {
