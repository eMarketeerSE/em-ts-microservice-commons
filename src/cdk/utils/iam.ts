...
import { Effect, PolicyDocument, PolicyStatement, Role, ServicePrincipal, ManagedPolicy } from 'aws-cdk-lib/aws-iam'
...
export const createSQSAccessPolicy = (
  queueArn: string,
  actions: string[] = ['sqs:SendMessage', 'sqs:ReceiveMessage', 'sqs:DeleteMessage', 'sqs:GetQueueAttributes']
): PolicyStatement => {
  return new PolicyStatement({
    effect: Effect.ALLOW,
    actions,
